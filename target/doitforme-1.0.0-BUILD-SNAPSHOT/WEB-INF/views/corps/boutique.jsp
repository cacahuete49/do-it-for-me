			...
			<div class="row">
				<div class="col-md-1">
					<img class="featurette-image center-block"
						data-src="holder.js/96x96" alt="img 96x96"
						src="resources/img/xml.png" data-holder-rendered="true"
						height="46px" />
				</div>
				<div class="col-md-8">
					<c:out value="${scenario.description}" />
					<p>
						Nombre de t&eacute;l&eacute;chargement:
						<c:out value="${fn:length(scenario.listeUser)}" />
					</p>
				</div>
				<div class="col-md-2">
					<f:form class="form" modelAttribute="scenario" method="POST"
						action="achat">
                        ...
                        Acheter <c:out value="${scenario.prix}" />&euro;
							...
							<c:otherwise>
								<f:button type="submit" class="btn btn-lg btn-info"
									action="achat"
									onclick="return confirm('Confirmez-vous l\\'achat ?');">
                        Acheter <c:out value="${scenario.prix}" />&euro;
                        </f:button>
							</c:otherwise>
						</c:choose>
					...
					<p>
						<c:forEach var="j" begin="1" end="5">
							<fmt:parseNumber var="note" type="number"
								value="${scenario.note}" />
